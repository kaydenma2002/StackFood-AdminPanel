...
class Helpers
{
    ...
    {
        return Cache::remember('schedule_order', 60, function () {
    ...
    public static function cart_product_data_formatting($data, $selected_variation, $selected_addons, $selected_addon_quantity, $trans = false, $local = 'en')
{
    ...
    if (!empty($data['category_ids'])) {
        ...
        $category_ids_array = array_column($category_ids, 'id');
        $categories_data = Category::whereIn('id', $category_ids_array)->pluck('name', 'id')->toArray();

        // Map over the category_ids to format them
        $categories = collect($category_ids)->map(function ($value) use ($categories_data) {
            return [
                'id' => (string) $value['id'],
                'position' => $value['position'],
                'name' => $categories_data[$value['id']] ?? 'NA'
            ];
        })->unique('id');
    }
    $data['category_ids'] = $categories->toArray();

    // Add-on data handling
    $add_ons = !empty($data['add_ons']) && is_array($data['add_ons']) ? $data['add_ons'] : json_decode($data['add_ons'], true);

    // Bulk fetch addons instead of querying in the loop
    $addons_data = AddOn::whereIn('id', $add_ons ?? [])->active()->get()->keyBy('id');

    $selected_data = array_combine($selected_addons ?? [], $selected_addon_quantity ?? []);

    // Map over the add-ons to format them
    $data['addons'] = $addons_data->map(function ($addon) use ($selected_addons, $selected_data) {
        $addon['isChecked'] = in_array($addon->id, $selected_addons ?? []);
        $addon['quantity'] = $addon['isChecked'] ? ($selected_data[$addon->id] ?? 0) : 0;
        return $addon;
    })->toArray();

    // Handling common data fields
    $data['name'] = $data->title ?? $data['name'];
    $data['available_time_starts'] = isset($data->available_time_starts) ? $data->available_time_starts: null;
    $data['available_time_ends'] = isset($data->available_time_ends) ? $data->available_time_ends : null;
    $data['available_date_starts'] = isset($data->start_date) ? $data->start_date->format('Y-m-d') : null;
    $data['available_date_ends'] = isset($data->end_date) ? $data->end_date->format('Y-m-d') : null;

    // Handling variations
    $data_variation = !empty($data['variations']) ? (is_array($data['variations']) ? $data['variations'] : json_decode($data['variations'], true)) : [];
    $data['variations'] = collect($data_variation)->map(function ($item2) use ($selected_variation) {
        $item2['values'] = collect($item2['values'])->map(function ($value) use ($item2, $selected_variation) {
            $selected_variation_item = collect($selected_variation)->firstWhere('name', $item2['name']);
            $value['isSelected'] = in_array($value['label'], $selected_variation_item['values']['label'] ?? []);
            return $value;
        })->toArray();
        return $item2;
    })->toArray();
    ...
    // Cuisines
    $data['cuisines'] = $data->restaurant->cuisine->map(function ($cuisine) {
        return [
            'id' => (int) $cuisine->id,
            'name' => $cuisine->name,
            'image' => $cuisine->image
        ...
        return Cache::remember($cacheKey, 60 * 60, function () use ($data, $multi_data, $trans, $local) {
            ...
            if ($multi_data) {
                if ($data instanceof Collection) {
                    $data->load([
                        'restaurant.discount', // Load discount along with restaurant
                        'restaurant.cuisine',
                ...
                $storage = $data->map(function ($item) use ($trans, $local) {
                    return self::format_single_product($item, $trans, $local);
                });
    ...
    private static function format_single_product($item, $trans, $local)
    {
        ...
        if ($item instanceof Model) {
            ...
            // Access the already loaded discount data
            $discount = $restaurant->discount;

            if ($discount && now()->between($discount->start_date, $discount->end_date)) {
                $item['restaurant_discount'] = $discount->discount;
            } else {
                $item['restaurant_discount'] = 0;
            }
    ...
    public static function restaurant_data_formatting($data, $multi_data = false)
    {
        ...
        return Cache::remember($cacheKey, $cacheDuration, function () use ($data, $multi_data) {
            ...
            if ($multi_data) {
                // Ensure $data is a collection of models
                if ($data instanceof \Illuminate\Support\Collection) {
                    ...
                    // Extract all restaurant IDs
                    $restaurantIds = $data->pluck('id')->toArray();

                    // Load coupons for all restaurants in a single query
                    $coupons = Coupon::where(function ($q) use ($restaurantIds) {
                        $q->where('coupon_type', 'restaurant_wise')
                            ->whereJsonContains('data', $restaurantIds)
                            ->whereJsonContains('customer_id', ['all']);
                    })->orWhereIn('restaurant_id', $restaurantIds)
                        ->active()
                        ->valid()
                        ->take(10)
                        ->get()
                    ...
                    foreach ($data as $item) {
                        ...
                        // Associate coupons with the restaurant
                        $item['coupons'] = $coupons->get($restaurant_id) ?? collect();

                        $item['self_delivery_system'] = (int) ($item->restaurant_model == 'subscription' && isset($item->restaurant_sub) ? $item->restaurant_sub->self_delivery : 0);
                        ...
                        // Calculate ratings
                        $reviewsInfo = $item->reviews->groupBy('food.restaurant_id')->first();
                        $reviewsInfo = collect($reviewsInfo);

                        $item['avg_rating'] = (float) $reviewsInfo->avg('rating') ?? 0;
                        $item['rating_count'] = (int) ($reviewsInfo->count() ?? 0);
                        $item['positive_rating'] = (int) RestaurantLogic::calculate_positive_rating($item['rating'])['rating'];
                        ...
                        unset($item['campaigns'], $item['pivot'], $item['rating'], $item['restaurant_config']);
                        array_push($storage, $item);
                    }
                ...
                if ($data instanceof \App\Models\Restaurant) { // Ensure $data is a model instance
                    ...
                    $data['coupons'] = Coupon::where(function ($q) use ($restaurant_id) {
                        $q->where('coupon_type', 'restaurant_wise')
                            ->whereJsonContains('data', [$restaurant_id])
                            ->whereJsonContains('customer_id', ['all']);
                    ...
                    $data['self_delivery_system'] = (int) ($data->restaurant_model == 'subscription' && isset($data->restaurant_sub) ? $data->restaurant_sub->self_delivery : 0);
                    ...
                    // Calculate ratings
                    $reviewsInfo = $data->reviews->groupBy('food.restaurant_id')->first();
                    $data['avg_rating'] = (float) ($reviewsInfo->avg('rating') ?? 0);
                    $data['rating_count'] = (int) ($reviewsInfo->count() ?? 0);
                    $data['positive_rating'] = (int) RestaurantLogic::calculate_positive_rating($data['rating'])['rating'];
                    ...
                    // Clean up unnecessary data
                    unset($data['rating'], $data['campaigns'], $data['pivot'], $data['restaurant_config']);
                }
            }
    ...
    {
        $cacheKey = "payment_method_config_{$name}";

        // Cache the result for 60 minutes (or any duration you prefer)
        $config = Cache::remember($cacheKey, 60, function () use ($name, $json_decode) {
            $paymentmethod = BusinessSetting::where('key', $name)->first();

            if ($paymentmethod) {
                return $json_decode ? json_decode($paymentmethod->value, true) : $paymentmethod->value;
            }
    ...
    {
        $order = Order::where('id', $order->id)->with('zone:id,deliveryman_wise_topic', 'restaurant:restaurant_id,name,restaurant_model,self_delivery_system,vendor_id', 'restaurant.restaurant_sub', 'customer:id,cm_firebase_token,email,f_name,l_name,current_language_key', 'restaurant.vendor:id,firebase_token', 'delivery_man:id,fcm_token', 'guest')->first();
        $push_notification_status = self::getNotificationStatusData('restaurant', 'restaurant_order_notification');
        $restaurant_push_notification_status = self::getRestaurantNotificationStatusData($order?->restaurant?->restaurant_id, 'restaurant_order_notification');
    ...
    public static function get_restaurant_id()
    {
        ...
        CountLog::info('auth vendor' . auth('vendor')->user());
    ...
    public static function get_full_url($path, $data, $type, $placeholder = null)
    {
        ...
        // API requests might need a null response instead of a placeholder
        if (request()->is('api/*')) {
            return null;
        }

        // Return the appropriate placeholder image
    ...
    public static function subscription_plan_chosen($restaurant_id, $package_id, $payment_method, $discount, $reference = null, $type = null)
    {
        ...
        try {
            ...
            if (isset($restaurant_subscription) && $type == 'renew') {
                ...
                RestaurantSubscription::where('restaurant_id', $restaurant->restaurant_id)->update([
                    'status' => 0,
                ]);
            ...
            $restaurant_subscription->restaurant_id = $restaurant->restaurant_id;
            ...
            $subscription_transaction->restaurant_id = $restaurant->restaurant_id;
    ...
    public static function get_restaurant_name($restaurant)
    {
        if (is_array($restaurant)) {
            ...
            $data = Restaurant::where('restaurant_id', $restaurant)->pluck('name')->toArray();
        }
        $data = implode(', ', $data);
        return $data;
    }
    ...
    public static function getDeliveryFee($restaurant): string
    {
        ...
        // Cache the zone data to avoid duplicate queries
        static $zoneCache = [];

        if (!isset($zoneCache[$restaurant->zone_id])) {
            $zoneCache[$restaurant->zone_id] = Zone::where('id', $restaurant->zone_id)
                ->whereContains('coordinates', new Point($latitude, $longitude, POINT_SRID))
                ->first();
        }

        $zone = $zoneCache[$restaurant->zone_id];
        ...
        if (isset($restaurant->distance) && $restaurant->distance > 0) {
            ...
            $originCoordinates = [$restaurant->latitude, $restaurant->longitude];
            $destinationCoordinates = [$latitude, $longitude];
            $distance = round(self::get_distance($originCoordinates, $destinationCoordinates), 5);
        } else {
            return 'out_of_range';
        }
        ...
        if ($restaurant['self_delivery_system'] == 1) {
            if (
                $restaurant->free_delivery == 1 ||
                ($restaurant->free_delivery_distance_status == 1 && $distance <= $restaurant->free_delivery_distance_value)
            ) {
            ...
            $free_delivery_distance = BusinessSetting::where('key', 'free_delivery_distance')->value('value') ?? 0;
            ...
            $increased = $zone->increased_delivery_fee_status == 1 ? $zone->increased_delivery_fee ?? 0 : 0;
            $extra_charges = (float)(self::vehicle_extra_charge(distance_data: $distance)['extra_charge'] ?? 0);
        }

        $original_delivery_charge = max($distance * $per_km_shipping_charge + $extra_charges, $minimum_shipping_charge + $extra_charges);

        if ($increased > 0 && $original_delivery_charge > 0) {
            $increased_fee = ($original_delivery_charge * $increased) / 100;
            $original_delivery_charge += $increased_fee;
        }
                ...
                foreach ($variation_options as $variation_option) {
                    if ($variation_option->stock_type !== 'unlimited') {
                        ...
                        if ($availableStock <= 0 || $availableStock < $quantity) {
                            return [
                                'out_of_stock' => $availableStock > 0 ? translate('Only') . ' ' . $availableStock . " " . translate('Quantity_is_abailable_for') . ' ' . $product?->name . ' \'s ' . $variation_option->option_name . ' ' . translate('Variation_!!!') : $product?->name . ' \'s ' . $variation_option->option_name . ' ' . translate('Variation_is_out_of_stock_!!!'),
