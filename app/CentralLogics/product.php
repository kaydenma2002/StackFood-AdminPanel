...
class ProductLogic
{
    ...
    public static function recommended_products($zone_id, $restaurant_id, $limit = null, $offset = null, $type = 'all', $name = null)
    {
        ...
        $paginator = Food::where('restaurant_id', $restaurant_id)
            ->active()
            ->type($type)
            ->whereHas('restaurant', function ($q) use ($zone_id) {
                $q->whereIn('zone_id', $zone_id)->Weekday();
            ...
            ->when(isset($name), function ($query) use ($name) {
                if (is_array($name)) {
                    // Join array elements into a single string with a space between each element
                    $name = implode(' ', $name);
                }

                // Search for the entire combined string in `name` and `tag`
