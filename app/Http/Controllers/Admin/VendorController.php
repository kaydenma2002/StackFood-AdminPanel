...
class VendorController extends Controller
{
            ...
            if ($default_lang == $key && !($request->name[$index])) {
                if ($key != 'default') {
                    array_push($data, array(
                        ...
                        'translationable_id' => $restaurant->restaurant_id,
                        'locale' => $key,
                        'key' => 'name',
    ...
    public function update(Request $request, Restaurant $restaurant)
    {
        ...
        $restaurant->slug = $restaurant->slug ? $restaurant->slug : "{$slug}{$restaurant->restaurant_id}";
    ...
    {
        if (env('APP_MODE') == 'demo' && $restaurant->restaurant_id == 2) {
    ...
    public function view($restaurant, Request $request, $tab = null, $sub_tab = 'cash')
    {
        ...
        if ($tab == 'settings') {
            ...
            $orders = Order::where('restaurant_id', $restaurant->restaurant_id)->with('customer')
            ...
            $transcations = SubscriptionTransaction::where('restaurant_id', $restaurant->restaurant_id)->with('package')
            ...
            $disbursements = DisbursementDetails::where('restaurant_id', $restaurant->restaurant_id)
                ->when(isset($key), function ($q) use ($key) {
                    $q->where(function ($q) use ($key) {
                        foreach ($key as $value) {
    ...
    //     $transcations=SubscriptionTransaction::where('restaurant_id', $restaurant->restaurant_id)
    ...
    public function get_restaurants(Request $request)
    {
        ...
        $data = Restaurant::with('zone')->when($zone_ids, function ($query) use ($zone_ids) {
    ...
    public function status(Restaurant $restaurant, Request $request)
    {
        ...
        try {
            if ($request->status == 0) {
                ...
                $reataurant_push_notification_status = Helpers::getRestaurantNotificationStatusData($restaurant->restaurant_id, 'restaurant_account_block');
    ...
    public function restaurant_filter($id)
    {
        if ($id == 'all') {
            ...
            session()->put('restaurant_filter', Restaurant::where('restaurant_id', $id)->first(['id', 'name']));
        }
        return back();
    }
    ...
    {
        $validator =  Validator::make($request->all(), [
            'upload_excel' => 'required|max:4096',
        ]);
        if ($validator->fails()) {
            return back()->withErrors($validator);
        }
        ...
        if ($request->button === 'import') {
            ...
            try {
                foreach ($collections as $key => $collection) {
                    if (
                        !isset($collection['id'])   || $collection['id'] === "" ||  $collection['ownerFirstName'] === "" || $collection['restaurantName'] === ""  || $collection['Comission'] === "" || $collection['Tax'] === ""
                        || $collection['email'] === "" || $collection['zone_id'] === "" ||  $collection['DeliveryTime'] === ""  || $collection['RestaurantModel'] === ""
                    ) {
                        $missingFields = [];
                        ...
                        if ($collection['ownerFirstName'] === "") {
                            $missingFields[] = 'ownerFirstName';
                        }
                        if ($collection['restaurantName'] === "") {
                            $missingFields[] = 'restaurantName';
                        }
                        if ($collection['Comission'] === "") {
                            $missingFields[] = 'Comission';
                        }
                        if ($collection['Tax'] === "") {
                            $missingFields[] = 'Tax';
                        }
                        ...
                        if ($collection['zone_id'] === "") {
                            $missingFields[] = 'zone_id';
                        }
                        if ($collection['DeliveryTime'] === "") {
                            $missingFields[] = 'DeliveryTime';
                        }
                        if ($collection['RestaurantModel'] === "") {
                            $missingFields[] = 'RestaurantModel';
                        }

                        if (!empty($missingFields)) {
                            $missingFieldsString = implode(', ', $missingFields);
                            $errorMessage = translate('messages.please_fill_all_required_fields') . ': ' . $missingFieldsString;
                    ...
                    array_push($vendors, [
                        'id' => $vendor_id + $key + 1,
                        'f_name' => $collection['f_name'],
                        'l_name' => $collection['l_name'],
                        'password' => bcrypt(12345678),
                        'phone' => $collection['phone'] ?? 5715051131,
                    ...
                    array_push($restaurants, [
                        ...
                        'email' => $collection['email'],
                        'latitude' => $collection['latitude'] ?? 88,
                        'longitude' => $collection['longitude'] ?? 91,
            ...
            try {
                ...
                foreach ($chunk_restaurants as $key => $chunk_restaurant) {
                    Log::info("Processing chunk $key out of " . count($chunk_restaurants));
                    ...
                    foreach ($chunk_restaurant as $restaurant) {
                        ...
                        Log::info("Inserted restaurant with ID $insertedId");
                    }
                }
        ...
        if ($request->button === 'update') {
            ...
            try {
                foreach ($collections as $key => $collection) {
                    if (
                        !isset($collection['id'])   || $collection['id'] === "" ||  $collection['ownerFirstName'] === "" || $collection['restaurantName'] === "" || $collection['phone'] === "" || $collection['Comission'] === "" || $collection['Tax'] === ""
            ...
            try {
                ...
                foreach ($chunk_restaurants as $key => $chunk_restaurant) {
                    ...
                    foreach ($chunk_restaurant as $restaurant) {
                        if (isset($restaurant['restaurant_id']) && DB::table('food')->where('id', $restaurant['restaurant_id'])->exists()) {
                            DB::table('restaurants')->where('id', $restaurant['restaurant_id'])->update($restaurant);
                            Helpers::updateStorageTable(get_class(new Restaurant), $restaurant['restaurant_id'], $restaurant['logo']);
                            Helpers::updateStorageTable(get_class(new Restaurant), $restaurant['restaurant_id'], $restaurant['cover_photo']);
                        } else {
                            $insertedId = DB::table('restaurants')->insertGetId($restaurant);
                            Helpers::updateStorageTable(get_class(new Restaurant), $insertedId, $restaurant['logo']);
                            Helpers::updateStorageTable(get_class(new Restaurant), $insertedId, $restaurant['cover_photo']);
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        // Retrieve the vendors based on the request criteria
        ...
        $exportData = RestaurantLogic::format_export_restaurants(Helpers::Export_generator($vendors));

        // Use the ExportService to handle the export
        return $this->exportService->export($exportData, 'Restaurants.xlsx');
    }
    ...
    public function updateStoreMetaData(Restaurant $restaurant, Request $request)
    {
        ...
        Helpers::add_or_update_translations(request: $request, key_data: 'meta_title', name_field: 'meta_title', model_name: 'Restaurant', data_id: $restaurant->restaurant_id, data_value: $restaurant->meta_title);
        Helpers::add_or_update_translations(request: $request, key_data: 'meta_description', name_field: 'meta_description', model_name: 'Restaurant', data_id: $restaurant->restaurant_id, data_value: $restaurant->meta_description);
    ...
    public function download_pdf(Restaurant $restaurant)
    {
        ...
        //        );
        //        Helpers::gen_mpdf(view: $mpdf_view,file_prefix: 'qr-code' . rand(00001, 99999),file_postfix: $restaurant->restaurant_id);
