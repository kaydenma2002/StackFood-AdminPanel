...
class SubscriptionController extends Controller
{
    ...
    public function subscription_list(Request $request)
    {
        ...
        $restaurants = Restaurant::with(['restaurant_sub_update_application','vendor','restaurant_sub_update_application.package'])
        ->has('restaurant_sub_update_application')

        ->when($type == 'subscribed', function ($query) {
            return $query->has('restaurant_sub');
        })
        ->when($type == 'unsubscribed', function ($query) {
            return $query->whereDoesntHave('restaurant_sub_update_application',function($query){
                $query->where('status', '1');
            });
        })
        ->when($type == 'expire_soon', function ($query) {
            return $query->wherehas('restaurant_sub', function ($query) {
                        return $query->whereDate('expiry_date', '<=', Carbon::today()->addDays('10'));
                    });
        })
        ->when($request->query('search'),function($query)use($request){
            $query->where(function($query)use($request){
                $key = explode(' ', $request->query('search'));
                foreach ($key as $value) {
        ...
        $transcations=SubscriptionTransaction::withoutGlobalScope('translate')->whereHas('restaurant', function($query)use($request){
            return $query->where('zone_id', $request->zone_id);
        })->with('package')->get();
        $sub_transcations = $transcations->count();
        $total_earning = $transcations->sum('paid_amount');
        $this_month = SubscriptionTransaction::withoutGlobalScope('translate')->whereHas('restaurant', function($query)use($request){
            return $query->where('zone_id', $request->zone_id);
        })->with('package')->whereMonth('created_at', Carbon::now()->month)->sum('paid_amount');


        $total_restaurant = Restaurant::has('restaurant_sub_update_application')
        ->when(is_numeric($request->zone_id), function ($query) use ($request) {
            return $query->where('zone_id', $request->zone_id);
        })->count();
        $total_active_subscription = Restaurant::has('restaurant_sub')
        ->when(is_numeric($request->zone_id), function ($query) use ($request) {
            return $query->where('zone_id', $request->zone_id);
        })->count();
        ...
        ->whereDoesntHave('restaurant_sub_update_application',function($query){
            return $query->where('status', '1');
        })->count();
        $expire_soon = Restaurant::wherehas('restaurant_sub', function ($query) {
            return $query->whereDate('expiry_date', '<=', Carbon::today()->addDays('10'));
        })
        ->when(is_numeric($request->zone_id), function ($query) use ($request) {
            return $query->where('zone_id', $request->zone_id);
    ...
    public function settings_update(Request $request){
        ...
        if($request->type == 'year'){
            $free_trial_period = $request->free_trial_period * 365;
        } else if($request->type == 'month'){
            $free_trial_period = $request->free_trial_period * 30;
        } else{
            $free_trial_period = $request->free_trial_period;
        }

        $values= [
            'data' => $free_trial_period,
            'type' => $request->type,
            'status' => $data['status'],
        ];
        // dd($values);
    ...
    public function transcation_list_export(Request $request){
        try{
            ...
            elseif($request->restaurant_id){
                $restaurant_name= Restaurant::where('restaurant_id',$request->restaurant_id)->first()?->name;
            }
            $data = [
                'data'=>$data,
