...
class VendorController extends Controller
{
    ...
    if ($Payable_Balance == 1 &&  $cash_in_hand_overflow &&  $vendor?->wallet?->balance < 0 &&  $cash_in_hand_overflow_restaurant_amount < abs($vendor?->wallet?->collected_cash)){
        ...
        $st = Restaurant::withoutGlobalScope('translate')->findOrFail($restaurant['restaurant_id']);
        ...
        if($restaurant->restaurant_model == 'subscription'){
            if(isset($restaurant?->restaurant_sub)){
                ...
                $vendor['subscription'] =RestaurantSubscription::where('restaurant_id',$restaurant->restaurant_id)->with('package')->latest()->first();
                $vendor['subscription_other_data'] =  [
                    'total_bill'=>  (float) SubscriptionTransaction::where('restaurant_id', $restaurant->restaurant_id)->where('package_id', $vendor['subscription']?->package?->id)->sum('paid_amount'),
    ...
    Public function make_payment(Request $request){
        ...
        $restaurant =Restaurant::findOrfail($restaurant->restaurant_id);
