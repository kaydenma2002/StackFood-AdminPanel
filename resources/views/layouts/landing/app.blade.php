...
<head>
    ...
    @php($icon_storage = \App\CentralLogics\Helpers::get_settings_storage('icon'))
    <link rel="shortcut icon" type="image/x-icon" href="{{ \App\CentralLogics\Helpers::get_full_url('business',$icon,$icon_storage) }}">
    ...
    @if (isset($backgroundChange['primary_1_hex']) && isset($backgroundChange['primary_2_hex']))
        <style>
            :root {
                --base-1: {{ $backgroundChange['primary_1_hex'] }};
                --base-rgb: {{ $backgroundChange['primary_1_rgb'] }};
                --base-2: {{ $backgroundChange['primary_2_hex'] }};
                --base-rgb-2: {{ $backgroundChange['primary_2_rgb'] }};
                --base-1-rgb: {{ $backgroundChange['primary_2_rgb'] }};
                --base-15-rgb: {{ $backgroundChange['primary_2_rgb'] }};
            }
        </style>
    @endif
    @stack('css_or_js')
...
<body>
    ...
    <?php
        ...
        $logo = \App\CentralLogics\Helpers::get_settings('logo');
        $logo_storage = \App\CentralLogics\Helpers::get_settings_storage('logo');
        $logo_url = \App\CentralLogics\Helpers::get_full_url('business', $logo, $logo_storage);

        $address = \App\CentralLogics\Helpers::get_settings('address');
        $email_address = \App\CentralLogics\Helpers::get_settings('email_address');
        $phone = \App\CentralLogics\Helpers::get_settings('phone');
        $footer_text = \App\CentralLogics\Helpers::get_settings('footer_text');
                                ...
                                <div class="input--grp">
                                    ...
                                    <button class="search-btn" type="submit">
                                        <svg width="46" height="47" viewBox="0 0 46 47" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <rect y="0.349609" width="46" height="46" rx="23" fill="url(#paint0_radial_246_199)" />
                                            <path
                    ...
                    <div class="footer-widget widget-links">
                        ...
                        <ul>
                            ...
                            @if (isset($landing_data['shipping_policy_status']) && $landing_data['shipping_policy_status']  == 1)
                                <li>
                                    <a href="{{ route('shipping-policy') }}">{{ translate('messages.shipping_policy') }}</a>
                                </li>
                            @endif
                            @if (isset($landing_data['refund_policy_status']) && $landing_data['refund_policy_status']  == 1)
                                <li>
                                    <a href="{{ route('refund-policy') }}">{{ translate('messages.refund_policy') }}</a>
                                </li>
                            @endif
                            @if (isset($landing_data['cancellation_policy_status']) && $landing_data['cancellation_policy_status']  == 1)
                                <li>
                                    <a href="{{ route('cancellation-policy') }}">{{ translate('messages.cancellation_policy') }}</a>
                                </li>
                            @endif
                        </ul>
                    </div>
