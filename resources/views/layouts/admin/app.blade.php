...
@php
    // Fetch the admin_order_notification setting once
    $admin_order_notification = \App\Models\BusinessSetting::where('key', 'admin_order_notification')->first();
    $admin_order_notification_value = $admin_order_notification ? $admin_order_notification->value : 0;

    // Fetch the order_notification_type setting once
    $order_notification_type = \App\Models\BusinessSetting::where('key', 'order_notification_type')->first();
    $order_notification_type_value = $order_notification_type ? $order_notification_type->value : 'firebase';
@endphp
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="text-center">
                                    ...
                                    <hr>
                                    <button class="btn btn-primary check-message">{{ translate('messages.Ok_let_me_check') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        messaging.onMessage(function(payload) {
            ...
            if (payload.data.order_id && payload.data.type === "order_request") {
                if (adminOrderNotification && orderNotificationType === 'firebase') {
        ...
        @if(\App\CentralLogics\Helpers::module_permission_check('order') && $order_notification_type_value === 'manual')
            if (adminOrderNotification) {
