@foreach ($restaurants as $key => $restaurant)
    <tr>
        <td>{{ $key + 1 }}</td>
        <td>
            <a href="{{ route('admin.restaurant.view', $restaurant->restaurant_id) }}" alt="view restaurant" class="table-rest-info">
            ...
            @if ($restaurant->pivot && $restaurant->pivot->campaign_status == 'pending')
                <div class="btn--container justify-content-center">
                    <a class="btn btn-sm btn--primary btn-outline-primary action-btn status-change-alert"
                       data-url="{{ route('admin.campaign.restaurant_confirmation', [$campaign->id, $restaurant->restaurant_id, 'confirmed']) }}" data-message="{{ translate('messages.you_want_to_confirm_this_restaurant') }}"
                    ...
                    <a class="btn btn-sm btn--danger btn-outline-danger action-btn status-change-alert" href="javascript:"
                       data-url="{{ route('admin.campaign.restaurant_confirmation', [$campaign->id, $restaurant->restaurant_id, 'rejected']) }}" data-message="{{ translate('messages.you_want_to_reject_this_restaurant') }}" data-toggle="tooltip" data-placement="top" title="{{translate('Deny')}}">
            ...
            @else
                <div class="btn--container justify-content-center">
                    <a class="btn btn-sm btn--danger btn-outline-danger action-btn form-alert" href="javascript:"
                        data-id="restaurant-{{ $restaurant['restaurant_id'] }}" data-message="{{ translate('Want_to_remove_this_restaurant') }}"
                        title="{{ translate('messages.delete_restaurant') }}"><i
                            class="tio-delete-outlined"></i>
                    </a>
                    <form action="{{ route('admin.campaign.remove-restaurant', [$campaign->id, $restaurant['restaurant_id']]) }}"
                        method="get" id="restaurant-{{ $restaurant['restaurant_id'] }}">
                        @csrf @method('get')
                    </form>
                </div>
            @endif
